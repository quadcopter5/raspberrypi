...
int main(int argc, char **argv) {
	...
	uart_base = mmap(NULL, BCM2835_PAGE_SIZE, PROT_READ | PROT_WRITE,
	...
	printf("GPIO library initialized (testing for compatibility)\n");
...
#define SPS  0x80  // Stick parity select
	...
	uint32_t lcrhval = PEN | EPS | FEN | WL8;
	*lcrh = lcrhval;

	printf("UART line control settings set\n");

	uint32_t *maskint = (uint32_t *)(uart_base + UART_OFFSET_IMSC);
	*maskint = 0x000007B0; //0x000007F3; 0x000007B0
	uint32_t maskval = *maskint;
	printf("Contents of IMSC: 0x%08X\n", maskval);

	uint32_t *mis = (uint32_t *)(uart_base + UART_OFFSET_MIS);
	uint32_t misval = *mis;
	printf("Contents of MIS: 0x%08X\n", misval);
...
//	*ibaud = 0x00000030; // 9600 baud
	...
	*cr |= 0x0001; // UART enable
	...
	// Write to Data Register (should initiate transmission)
...
#define UART_FR_RXFE 0x0010
	...
	*cr = 0x0000;
	...
	// Close mapping
	if (munmap(uart_base, BCM2835_PAGE_SIZE) < 0) {
